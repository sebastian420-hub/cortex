...
def generate_report(prompt: str, sections: List[Dict], args) -> None:
    ...
    for section in sections:
        ...
        print(
            f"{section['name']:<25} {section['lines']:>6} {section['chars']:>10,} "
    ...
    for name in candidate_sections:
        ...
        if section:
            ...
            print(
                f"  * {name}: {section['lines']} lines, {section['tokens']:,} tokens ({pct:.1f}%)"
            )

    print()
    print(f"Potential reduction: {candidate_lines} lines ({candidate_tokens:,} tokens)")
    print(
        f"New total would be: {total_lines - candidate_lines} lines, "
    ...
    if args.save:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            for section in sections:
                f.write(
                    f"- {section['name']}: {section['lines']} lines, "
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="gpt-4", help="Model for token counting (default: gpt-4)"
    )
