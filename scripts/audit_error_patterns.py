...
def analyze_file(file_path: Path) -> Dict[str, Any]:
    ...
    if not analysis["imports_errors_module"]:
        ...
        analysis["recommendations"].append(
            "Add: from ..utils.errors import create_error_response, create_success_response, ErrorType"
        )
...
def generate_report(analyses: List[Dict[str, Any]]) -> str:
    ...
    report_lines.append(
        f"- Total manual error returns: {sum(a['manual_error_returns'] for a in analyses)}"
    )
    report_lines.append("")
    ...
    for analysis in sorted(analyses, key=lambda x: x["compliance_score"]):
        ...
        report_lines.append(
            f"### {status} {Path(analysis['file']).name} ({analysis['compliance_score']}/100)"
        )
        report_lines.append(f"**File**: `{analysis['file']}`")
        report_lines.append(f"**Imports errors module**: {analysis['imports_errors_module']}")
        report_lines.append(
            f"**Uses create_error_response**: {analysis['uses_create_error_response']}"
        )
        ...
        if analysis["error_patterns"]:
            ...
            for pattern in analysis["error_patterns"][:5]:  # Limit to first 5
                report_lines.append(
                    f"  - Line {pattern['line']}: {pattern['pattern']} - `{pattern['code']}`"
                )
    ...
    if high_priority:
        ...
        for analysis in sorted(high_priority, key=lambda x: x["compliance_score"]):
            report_lines.append(
                f"- `{Path(analysis['file']).name}`: {analysis['compliance_score']}/100"
            )
            if analysis["issues"]:
    ...
    for analysis in sorted(
        [a for a in analyses if a["compliance_score"] < 70], key=lambda x: x["compliance_score"]
    ):
