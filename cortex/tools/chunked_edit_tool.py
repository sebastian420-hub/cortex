...
from ..core.memory_chunked.chunk import EditChunk, ChunkType, create_file_chunk, create_source_code_chunk
from ..core.memory_chunked.chunking import FileChunker, ChunkingStrategy, should_chunk_file
from ..core.memory_chunked.context_window import ContextWindowManager, create_context_window_from_file
...
class ChunkedEditTool(Tool):
    ...
    def execute(
        ...
        **kwargs
        ...
        try:
            ...
            should_chunk = should_chunk_file(open(full_path, 'r').read(), path) if file_size < 1000000 else True
            ...
            if chunk_id:
                ...
                return self._edit_file_chunked(
                    path, old_string, new_string, full_path
                )
            else:
                # Standard edit for small files
                return self._edit_file_standard(
                    path, old_string, new_string, full_path
                )
            ...
            return create_success_response(
                {
                    "path": path,
                    "changes": 1,
                    "bytes_modified": len(new_content) - len(content)
                }
            )
    ...
    def edit_chunk_content(
        ...
        **kwargs
        ...
        try:
            ...
            return self._edit_chunk_surgically(
                path, chunk_id, None, new_content, full_path
            )
    ...
    def rollback_last_edit(self) -> Dict[str, Any]:
        ...
        if not self.operation_history:
            return create_error_response(
                "No edit operations to rollback",
                ErrorType.VALIDATION
            )
        ...
        return create_success_response(
            {
                "rolled_back": last_op,
                "remaining_operations": len(self.operation_history)
            }
        )
