...
class Skill:
    ...
    def is_applicable(self, task_description: str, context: Optional[Dict[str, Any]] = None) -> float:
        """
...
class SkillLoaderTool(Tool):
    ...
    def _parse_skill_file(self, file_path: Path) -> Skill:
        ...
        elif any(word in file_lower or word in content_lower for word in ['migrate', 'api', 'upgrade']):
            category = SkillCategory.MIGRATION
        elif any(word in file_lower or word in content_lower for word in ['optimize', 'performance']):
