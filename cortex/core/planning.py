...
class PlanningEngine:
    ...
    def execute_plan(
        ...
        while True:
            ...
            if not ready_steps:
                ...
                all_completed = all(
                    step.status in [PlanStepStatus.COMPLETED, PlanStepStatus.SKIPPED, PlanStepStatus.FAILED]
    ...
    def _execute_step(self, step: PlanStep, plan: Plan) -> Dict[str, Any]:
        ...
        elif step.step_type == PlanStepType.SKILL_APPLICATION and step.skill_name and self.skill_loader:
            # Apply a skill
    ...
    def reflect_on_progress(
        ...
        skill_steps = [step for step in plan.steps if step.step_type == PlanStepType.SKILL_APPLICATION]
        if skill_steps:
