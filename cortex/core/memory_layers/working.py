...
class WorkingMemory:
    ...
    def get_files(self) -> List[FileContext]:
        ...
        for item in self.items.values():
            if item.type == WorkingMemoryItemType.FILE_CONTEXT and isinstance(
                item.content, FileContext
            ):
                item.touch()
    ...
    def get_tools(self) -> List[ToolContext]:
        ...
        tools = []
        for item in self.items.values():
            if item.type == WorkingMemoryItemType.TOOL_CONTEXT and isinstance(
                item.content, ToolContext
            ):
                item.touch()
    ...
    def get_summary(self) -> str:
        ...
        if files:
            file_list = [
                (
                    f"- {f.path}: {f.summary[:50]}..."
                    if len(f.summary) > 50
                    else f"- {f.path}: {f.summary}"
                )
                for f in files[:3]
            ]
            if len(files) > 3:
        ...
        if tools:
            tool_list = [
                (
                    f"- {t.tool_name}: {t.purpose[:50]}..."
                    if len(t.purpose) > 50
                    else f"- {t.tool_name}: {t.purpose}"
                )
                for t in tools[:2]
            ]
            if len(tools) > 2:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "WorkingMemory":
        ...
        wm.items = {k: WorkingMemoryItem.from_dict(v) for k, v in data.get("items", {}).items()}
    ...
    def _evict_if_needed(self) -> None:
        ...
        if len(self.items) > self.max_items:
            ...
            items_list.sort(key=lambda x: (x[1].priority, x[1].accessed_at))
