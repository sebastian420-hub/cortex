...
class DelegationRules:
    ...
    def _compile_patterns(self):
        ...
        self.security_pattern = re.compile(
            r"\b(" + "|".join(self.SECURITY_KEYWORDS) + r")\b", re.IGNORECASE
        )
        self.complexity_pattern = re.compile(
            r"\b(" + "|".join(self.COMPLEXITY_KEYWORDS) + r")\b", re.IGNORECASE
        )
        self.review_pattern = re.compile(
            r"\b(" + "|".join(self.REVIEW_KEYWORDS) + r")\b", re.IGNORECASE
        )
        self.implementation_pattern = re.compile(
            r"\b(" + "|".join(self.IMPLEMENTATION_KEYWORDS) + r")\b", re.IGNORECASE
        )
        self.simple_pattern = re.compile(
            r"\b(" + "|".join(self.SIMPLE_KEYWORDS) + r")\b", re.IGNORECASE
        )
    ...
    def _check_prohibited_delegations(
        self, step: ModelAwarePlanStep, current_model: str, violations: List[RuleViolation]
    ):
        """Check for prohibited delegation patterns."""

        # Rule 1: Don't delegate simple tasks
        if (
            self._is_simple_task(step)
            and step.required_model
            and step.required_model != current_model
        ):
            violations.append(
                RuleViolation(
        ...
        if (
            self._is_context_switching_task(step)
            and step.required_model
            and step.required_model != current_model
        ):
            violations.append(
    ...
    def _check_recommended_assignments(
        self, step: ModelAwarePlanStep, violations: List[RuleViolation]
    ):
