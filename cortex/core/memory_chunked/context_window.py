...
class ContextWindowManager:
    ...
    def inject_context(
        ...
        if needed_tokens > remaining:
            logger.warning(
                f"Selected chunks need {needed_tokens} tokens, but only {remaining} available"
            )
        ...
        if self.budget_per_operation:
            if not self.token_budget.allocate(needed_tokens, operation_type):
                ...
                return messages, {
                    "injected": 0,
                    "skipped": len(self.available_chunks),
        ...
        injection_info = {
            ...
            "budget_remaining": (
                self.token_budget.get_remaining()
                if self.budget_per_operation
                else remaining - needed_tokens
            ),
            "chunks": [chunk.chunk_id for chunk in chunks_to_inject],
        }

        logger.info(
            f"Injected {len(chunks_to_inject)} chunks ({needed_tokens} tokens) for task: {task}"
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "budget_remaining": (
                self.token_budget.get_remaining() if self.budget_per_operation else self.max_tokens
            ),
            "utilization": (
                sum(chunk.token_estimate for chunk in self.active_chunks) / self.max_tokens
            )
            * 100,
        }

    def _select_chunks(
        self, task: Optional[str], budget: int, operation_type: str
    ) -> List[EditChunk]:
    ...
    def _select_relevant_chunks(self, task: Optional[str], budget: int) -> List[EditChunk]:
    ...
    def _select_lru_chunks(self, budget: int) -> List[EditChunk]:
        ...
        sorted_chunks = sorted(
            self.available_chunks, key=lambda c: self.chunk_access_times.get(c.chunk_id, 0)
        )
    ...
    def _select_budget_aware_chunks(self, budget: int) -> List[EditChunk]:
        ...
        sorted_chunks = sorted(self.available_chunks, key=lambda c: c.token_estimate, reverse=True)
    ...
    def _score_chunk_type_relevance(self, chunk: EditChunk, operation_type: str) -> float:
        """
    ...
    def _update_relevance_scores(self, task: str, chunks: List[EditChunk]) -> None:
...
def estimate_context_usage(
    context_window: ContextWindowManager, messages: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
