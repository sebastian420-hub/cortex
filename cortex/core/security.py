...
def _check_dangerous_paths(tokens: List[str]) -> bool:
    ...
    read_only_commands = ['cat', 'less', 'more', 'head', 'tail', 'grep', 'find', 'ls', 'stat', 'file']
        ...
        if command in variations:
            ...
            if dangerous_cmd == 'rm':
                ...
                has_relative_path = any(
                    arg.startswith('./') or 
                    arg.startswith('../') or 
                    (not arg.startswith('/') and not arg.startswith('C:') and ':' not in arg)
                    for arg in tokens[1:] if not arg.startswith('-')
                )
...
def is_dangerous_command(command: str) -> bool:
    ...
    destructive_commands = ['rm', 'remove', 'del', 'delete', 'erase', 'format', 'mkfs', 'dd', 'chmod', 'chown']
