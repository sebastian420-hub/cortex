...
class ModelPricing:
    ...
    def estimate_cost(
        ...
        input_tokens: int,
        output_tokens: int,
        include_context_overhead: bool = True
    ) -> float:
        """
...
class CostTracker:
    ...
    def record_usage(
        ...
        if actual_cost_usd is None and actual_input_tokens is not None and actual_output_tokens is not None:
            pricing = self.pricing_registry.get_pricing(model_name, provider)
            if pricing:
                actual_cost_usd = pricing.estimate_cost(
    ...
    def _log_record(self, record: CostRecord) -> None:
        ...
        if record.actual_cost_usd:
            ...
            log_message = (
                f"Usage recorded: model={record.model_name}, "
                f"provider={record.provider}"
            )
