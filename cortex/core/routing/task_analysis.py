...
class TaskAnalysisEngine:
    ...
    def _classify_task(self, user_input: str) -> Tuple[TaskType, List[str], float]:
        ...
        top_tasks = [task_type for task_type, score in task_scores.items() if score == max_score]
    ...
    def _estimate_complexity(self, user_input: str) -> TaskComplexity:
        ...
        simple_matches = sum(
            1 for pattern in self.complexity_patterns["simple"] if re.search(pattern, user_input)
        )
        if simple_matches > 0:
            score -= 1
        ...
        complex_matches = sum(
            1 for pattern in self.complexity_patterns["complex"] if re.search(pattern, user_input)
        )
        ...
        urgent_matches = sum(
            1 for pattern in self.complexity_patterns["urgent"] if re.search(pattern, user_input)
        )
        if urgent_matches > 0:
        ...
        return TaskComplexity(score=score, reasoning=reasoning, factors=factors)
    ...
    def _create_unknown_analysis(self, user_input: str) -> TaskAnalysis:
        ...
        return TaskAnalysis(
            ...
            complexity=TaskComplexity(
                score=1, reasoning="Empty or invalid input", factors={"empty_input": True}
            ),
            keywords=[],
            confidence=0.1,
            raw_input=user_input,
