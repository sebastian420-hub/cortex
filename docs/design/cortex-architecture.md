# Cortex - Complete Architecture & Technical Specification
...
Cortex is a **unified agent for coding, cybersecurity, and personal assistance** that supports both local LLM models (via Ollama) and cloud APIs. It enables developers to interact with their codebase, security tools, and third-party services through natural language, with support for MCP server integration.
...
### 3.1 Cortex Class (Core Orchestrator)
...
Tools are now implemented as separate classes inheriting from a base `Tool` class, located in `cortex/tools/`:
...
**Available Tools (in `cortex/tools/`):**
...
docker run --rm -it \
  -v $(pwd):/workspace \
  -w /workspace \
  python:3.11 python cortex.py

# Or use firejail
firejail --private --net=none python cortex.py
```
...
agent = Cortex(model="llama3.2")  # 3B

# Production: Use large model
agent = Cortex(model="llama3.3:70b")  # 70B

# Balanced: Medium model
agent = Cortex(model="qwen2.5:32b")  # 32B
```
...
### Cortex vs Claude Code vs Alternatives

| Feature | Cortex | Claude Code | Cursor | Aider |
|---------|-----------|-------------|--------|-------|
...
Cortex demonstrates that a **Claude Code-like experience is achievable with local models** and relatively simple architecture. The key insights:
