...
class TestGlobTool:
    ...
    def test_recursive_glob(self, temp_project):
        ...
        assert result["count"] >= 4  # main.py, utils.py, src/app.py, src/config.py, tests/test_main.py
...
class TestEditTool:
    ...
    def test_edit_string_not_found(self, temp_project):
        ...
        assert result["success"] is False
        assert "not found" in result.get("error", "").lower() or "validation" in result.get("error_type", "")
    ...
    def test_edit_non_unique_string(self, temp_project):
        ...
        result = tool.execute(
            file_path="main.py",
            old_string="def",  # Appears multiple times
            new_string="function"
        )
        assert result["success"] is False
