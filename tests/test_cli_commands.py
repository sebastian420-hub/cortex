...
def mock_agent():
    ...
    ]  # A simple history  # noqa: E501
    agent.conversation.update_model = MagicMock()  # Mock the method

    agent._get_system_prompt.return_value = (
        "You are Cortex updated system prompt."  # Mock system prompt update  # noqa: E501
    )
...
def test_handle_model_switch_success(mock_agent, mock_repl):
    ...
    mock_agent.switch_model.side_effect = lambda new_m, provider_o: setattr(
        mock_agent, "model", new_m
    )  # noqa: E501
    ...
    with patch.object(console, "print") as mock_console_print:
        ...
        assert (
            mock_agent.conversation.history[0]["content"]
            == mock_agent._get_system_prompt.return_value
        )  # noqa: E501
